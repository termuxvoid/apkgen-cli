...
# Native C specific: update JNI function name in native-lib.c
update_native_c() {
    local c_file="$1/app/src/main/cpp/native-lib.c"
    [ -f "$c_file" ] || return
    local old_pkg="$2"
    local new_pkg="$3"
    ...
    sed -i "s/$old_jni/$new_jni/g" "$c_file"
}
...
create_nativec() {
    local app_dir="$1"
    local template_dir="$TEMPLATE_BASE/nativec"
    ...
    echo
    spinner "Copying nativec template..." cp -r "$template_dir" "$app_dir"
    if [ $? -ne 0 ]; then
        error "Copy failed"
    fi
    ...
    update_native_c "$(pwd)" "$DEFAULT_PACKAGE" "$new_pkg"
