...
# ------------------------------
# CREATE NATIVECPP APP
# ------------------------------
create_nativecpp() {
    local app_dir="$1"
    local template_dir="$TEMPLATE_BASE/nativecpp"
    ...
    echo
    spinner "Copying nativecpp template..." cp -r "$template_dir" "$app_dir"
    if [ $? -ne 0 ]; then
        error "Copy failed"
    fi
